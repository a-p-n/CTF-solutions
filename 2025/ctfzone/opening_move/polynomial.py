# This file contains the polynomial operations
import random
from ff import Fr
import copy
import unittest
...
def batch_polynomials(polynomials, batching_scalar, starting_scalar=Fr(1)):
    """Batch polynomials: P1 + batching_scalar * P2 + batching_scalar^2 * P3 + ... + batching_scalar^(n-1) * Pn"""
...
def batch_inverse(polynomial):
    """Batch inverse: 1/x1, 1/x2, 1/x3, ... , 1/xn"""
    products = [Fr(1)]
...
def vanishing_polynomial_on_domain(domain: list[Fr]):
    """Vanishing polynomial: (x - x1) * (x - x2) * ... * (x - xn)"""
    vanishing_poly = [Fr(1)]
...
def divide_polynomial_by_known_root(polynomial: list[Fr], root: Fr):
    """Divide a polynomial by a known root."""
    if root == 0:
...
def convert_from_lagrange_to_monomial_form(polynomial_in_evaluation_form: list[Fr]):
    """Convert a polynomial from evaluation form to monomial form."""
...
def partially_evaluate_multilinear_polynomial(polynomial, evaluation_point):
    """Partially evaluate a multilinear polynomial at a coordinate."""
    new_polynomial = []
...
def evaluate_multilinear_polynomial(polynomial, evaluation_points):
    """Evaluate a multilinear polynomial at a list of coordinates."""
