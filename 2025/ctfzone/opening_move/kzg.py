# This file contains the KZG commitment scheme
import unittest
...
def convert_to_working_point(commitment):
    """Convert a commitment to a point that the py_ecc library can work with."""
    coordinates = []
...
def batch_commitments(commitments, batching_scalar, starting_scalar=Fr(1)):
    """Batch commitments: C1 + batching_scalar * C2 + batching_scalar^2 * C3 + ... + batching_scalar^(n-1) * Cn"""
...
def map_to_optimized_fq(elements):
    """Map a list of elements to a list of FQ elements that the py_ecc library can work with."""
    return tuple(
        map(
            optimized_bn128_FQ,
...
def msm(scalars_and_points):
    """Multi-scalar multiplication: s1 * P1 + s2 * P2 + ... + sn * Pn"""
...
class KZG:
    def __init__(self, srs_file="./ronk_srs.bin"):
        """Initialize the KZG commitment scheme."""
    ...
    def open(self, polynomial, x):
        """Open a polynomial at a point x."""
        running_power = Fr(1)
        result = Fr(0)
    ...
    def verify(self, commitment, opening):
        """Verify a proof of a polynomial at a point x."""
        if len(commitment) == 2:
